...
def display():
    ...
    generator.load_state_dict(torch.load('./acgan_models/generator_c.pth', map_location='cpu'))
    generator.eval()
    ...
    save_image(fake.detach().data, 'generated_cifar10.png', 10, normalize=True)

    img = plt.imread('generated_cifar10.png')
    plt.imshow(img)
    plt.show()
...
def train(args, train_loader, device, Tensor, LongTensor):
    ...
    if args.resume:
        generator.load_state_dict(torch.load('./acgan_models/generator_c.pth'))
        discriminator.load_state_dict(torch.load('./acgan_models/discriminator_c.pth'))
    ...
    torch.save(generator.state_dict(), './acgan_models/generator_c.pth')
    torch.save(discriminator.state_dict(), './acgan_models/discriminator_c.pth')
